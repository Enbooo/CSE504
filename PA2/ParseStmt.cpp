	...
	if (peekIsOneOf({Token::Key_int, Token::Key_char}))
	{
		...
		try
		{
			...
			if (peekAndConsume(Token::Assign))
			{
				...
				else if (ident->getType() == Type::Int && assignExpr->getType() == Type::Char)
				...
				else if (ident->getType() != assignExpr->getType())
				{
					...
					reportSemantError("Cannot assign an expression of type " + fromType + " to " + toType, coln);
				}
...
shared_ptr<ASTCompoundStmt> Parser::parseCompoundStmt(bool isFuncBody)
{
	...
	if (peekAndConsume(Token::LBrace)) {
		...
		if (isFuncBody) {
			...
			if (!lstPt && mCurrReturnType == Type::Void) {
		...
		if (peekAndConsume(Token::Assign))
		{
			...
			if (arraySub)
			{
				...
				if (mCheckSemant && subType != expr->getType())
				{
					...
					else if (expr->getType() != Type::Void)
					{
			...
			else
			{
				...
				if (ident->isArray())
					...
					expr = intToChar(expr);
				else if (ident->getType() == Type::Int && expr->getType() == Type::Char)
					expr = charToInt(expr);
				else if (ident->getType() != expr->getType() && expr->getType() != Type::Void)
				{
...
shared_ptr<ASTReturnStmt> Parser::parseReturnStmt()
{
	...
	if (peekToken() == Token::Key_return) {
        ...
        if (exp && mCurrReturnType == Type::Char && exp->getType() == Type::Int) {
            exp = intToChar(exp);
        } else if (exp && mCurrReturnType == Type::Int && exp->getType() == Type::Char) {
			exp = charToInt(exp);
		}
        else if (!exp && mCurrReturnType != Type::Void) {
            reportSemantError("Invalid empty return in non-void function", coln);
        }        
		else if (exp && mCurrReturnType != exp ->getType()) {
