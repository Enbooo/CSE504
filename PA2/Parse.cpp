...
std::shared_ptr<ASTExpr> Parser::charToInt(std::shared_ptr<ASTExpr> expr) noexcept
{
	...
	{
		ASTConstantExpr *constExpr = dynamic_cast<ASTConstantExpr*>(expr.get());
		if (constExpr)
		{
		...
		{
			ASTToIntExpr* toCharExpr = dynamic_cast<ASTToIntExpr*>(expr.get());
			if (toCharExpr)
				return toCharExpr->getChild();
			return make_shared<ASTToIntExpr>(expr);
		}
	}
	return retVal;
}
...
std::shared_ptr<ASTExpr> Parser::intToChar(std::shared_ptr<ASTExpr> expr) noexcept
{
	...
	if (expr->getType() == Type::Int)
	{
		...
		{
			ASTToCharExpr* toIntExpr = dynamic_cast<ASTToCharExpr*>(expr.get());
			if (toIntExpr)
				return toIntExpr->getChild();
			return make_shared<ASTToCharExpr>(expr);
		}
	}
